 ...
 * code that grows with the perk shop — a fully maxed build overflows what a QR
 * code can hold and what a phone camera can reliably read. So for sharing we
 * deflate that code and base64url it behind a short prefix, which keeps the QR
 * small and dense-enough to scan in one shot.
 *
 * Decoding is symmetric and also accepts a plain (uncompressed) code untouched,
 * so pasting a text code (or a freshly scanned compact one) both flow into the
 * same `importSave()`.
 */
...
/** turn any transfer code (compact or plain) back into the plain code importSave expects */
